...
/**
 * CHANGED
 * CreateVM method changed to include virtual networks
 * 
 * ORIGINAL
...
public class NetDatacenterBroker extends SimEntity {
	...
	protected void createVmsInDatacenterBase(int datacenterId) {
		...
		for(Host host : linkDC.getHostList()) {
		...
		for (int i = 0; i < 100; i++) {
			this.getAppCloudletList().add(
					new WorkflowApp(AppCloudlet.APP_Workflow, NetworkConstants.currentAppId, 0, linkDC.getVmList().size(), getId()));
		...
		for (AppCloudlet app : this.getAppCloudletList()) {
			...
			if (vmids != null) {
				if (!vmids.isEmpty()) {
					...
					WorkflowApp wapp = (WorkflowApp)app;
					
					wapp.createCloudletList(vmids, linkDC);
					for (int i = 0; i < app.numbervm; i++) {
	...
	private void CreateVMs(int datacenterId) {
		...
		List<String> vnetList = new ArrayList<String>();
		vnetList.add(null);
		vnetList.add("92.100.56.33/10");
		vnetList.add("93.23.123.30/8");
		vnetList.add(null);
		vnetList.add("94.100.56.33/12");
		vnetList.add("95.100.56.33/10");
		vnetList.add(null);
		vnetList.add("96.100.56.33/16");
		vnetList.add(null);
		
		Random rand = new Random();
		
		int numVM = linkDC.getHostList().size() * NetworkConstants.maxhostVM*3;
		
		Log.printLine("numVM to create: " + numVM);
		for (int i = 0; i < numVM; i++) {
			...
			NetworkCloudletSpaceSharedScheduler netCloudSched = new NetworkCloudletSpaceSharedScheduler();
		
			
			rand.nextInt(vnetList.size());
			String vnet = null;
			if(i%2!=0) {
				vnet = null;
			}else if(i%5==0) {
				...
				vnet="94.100.56.33/12";
			}else if(i%11 ==0) {
				vnet="95.100.56.33/10";
			}else if(i%2 == 0) {
				vnet="96.100.56.33/16";
			}else if(i%7==0) {
				vnet = "92.100.56.33/10";
			}
			VnetVm vm = new VnetVm(
					vmid,
					getId(),
					mips,
					pesNumber,
		...
		for(Vm vm : getVmList()) {
			...
			Log.printLine("" + vm.getId() + " : " + vm1.getVnet());
		}
