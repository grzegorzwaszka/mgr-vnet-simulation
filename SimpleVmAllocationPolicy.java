...
public class SimpleVmAllocationPolicy extends VmAllocationPolicy {
	 ...
	 * @post $none
	 */
	public SimpleVmAllocationPolicy(List<? extends Host> list) {
		super(list);
	...
	public boolean allocateHostForVm(Vm vm) {
		...
		List<Integer> hostId = new ArrayList<>();
		for(Host host : getHostList()) {
			...
			if (host.isSuitableForVm(vm)) {
				...
				try {
					...
					if (powerAfterAllocation != -1) {
						...
						Integer pktCount = vnetHostIds.get(host.getId());
						if(pktCount == null) {
							pktCount = 1;
						}
						double powerDiff = powerAfterAllocation - host.getPower();
						Log.printLine("Packet cost from host " + host.getId() + " to vm " + vm.getId() + " is " + getPowerofPacketTrace(host, vm, pktCount));
						if (powerDiff + getPowerofPacketTrace(host, vm, pktCount)< minPower) {
							minPower = powerDiff;
							allocatedHost = host;
						}
					}
	...
	private double getPowerofPacketTrace(VnetHost vhFrom, VnetVm vm2, int pktCount){
		...
		int aggCount = 0;
		int rootCount = 0;
		VnetHost vhTarget = null;
		for(Host vh : getHostList()) {
			VnetHost vh1 = (VnetHost)vh;
			if(vh1.getVmList().contains(vm2)) {
				vhTarget = vh1;
			}
		}
		if(vhFrom.sw == vhTarget.sw) {
			return 0.000002 * pktCount;
		}else {
			if(vhFrom.sw.uplinkswitches.get(0) == vhTarget.sw.uplinkswitches.get(0)) {
				return (0.00001 + 0.000002) * pktCount;
			}else {
				return (0.00001 + 0.00001 + 0.000002) * pktCount;
			}
		}
